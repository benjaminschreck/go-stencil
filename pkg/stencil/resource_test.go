...
func TestPreparedTemplateClose(t *testing.T) {
	...
	if tmpl.template.renderResources != nil {
		t.Error("Close() should release cached render resources")
	}

	// Closed templates should no longer be renderable.
	data := map[string]interface{}{
		"name": "World",
	}
	_, err = tmpl.Render(data)
	if err == nil {
		t.Error("Render after Close() should fail")
	}
}

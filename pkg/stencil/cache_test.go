...
func TestTemplateCache_Basic(t *testing.T) {
	...
	// Cache hits should return a fresh handle to the shared prepared template.
	if prepared1 == prepared2 {
		t.Error("Expected cached template hit to return a distinct borrowed handle")
	}
}
...
func TestTemplateCache_ClearDoesNotCloseBorrowedTemplate(t *testing.T) {
	cache := NewTemplateCache()
	templateData := createTestDocx(t, "{{name}}")

	prepared, err := cache.Prepare(bytes.NewReader(templateData.Bytes()), "test-key")
	if err != nil {
		t.Fatalf("Failed to prepare template: %v", err)
	}
	...
	if _, err := prepared.Render(TemplateData{"name": "Alice"}); err != nil {
		t.Fatalf("Borrowed template should remain usable after cache clear: %v", err)
	}
}
...
func TestTemplateCache_Remove(t *testing.T) {
	...
	if prepared2 == cachedPrepared2 {
		t.Error("Expected cache hit to return a distinct borrowed handle")
	}
}

func TestTemplateCache_RemoveDoesNotCloseBorrowedTemplate(t *testing.T) {
	cache := NewTemplateCache()
	templateData := createTestDocx(t, "{{name}}")

	prepared, err := cache.Prepare(bytes.NewReader(templateData.Bytes()), "key1")
	if err != nil {
		t.Fatalf("Failed to prepare template: %v", err)
	}
	...
	if _, err := prepared.Render(TemplateData{"name": "Alice"}); err != nil {
		t.Fatalf("Borrowed template should remain usable after cache remove: %v", err)
	}
}
...
func TestTemplateCache_CloseBorrowedHandleDoesNotPoisonCache(t *testing.T) {
	cache := NewTemplateCache()
	templateData := createTestDocx(t, "{{name}}")
	...
	borrowed, ok := cache.Get("key1")
	if !ok {
		t.Fatal("Cache should still serve a template after a borrowed handle is closed")
	}
	if borrowed == prepared {
		t.Fatal("Cache hit should return a fresh borrowed handle")
	}
	if _, err := borrowed.Render(TemplateData{"name": "Alice"}); err != nil {
		t.Fatalf("Borrowed cached handle should remain usable: %v", err)
	}
}
